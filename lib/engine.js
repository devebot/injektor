...
var Injektor = function Injektor(params) {
  ...
  function parseName(name, options) {
    ...
    var nameRef = {
      name: name,
      absoluteName: parts.slice().reverse().join(config.separator),
  ...
  function updateSuggestion(nameRef) {
    if (nameRef.name != nameRef.relativeName) {
      namestore[nameRef.relativeName] = namestore[nameRef.relativeName] || [];
      if (namestore[nameRef.relativeName].indexOf(nameRef.name) < 0) {
        namestore[nameRef.relativeName].push(nameRef.name);
      }
    }
  }
