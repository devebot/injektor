...
var Injektor = function Injektor(params) {
  ...
  function analyticName(name, options) {
    validateName(name);
    options = options || {};
    var parts = name.split(config.separator).reverse();
    if (!parts[1] && options.scope) parts[1] = options.scope;
    var nameRef = {
      absoluteName: parts.join(config.separator),
      relativeName: parts[0],
      scope: parts[1]
    }
    if (name != nameRef.relativeName) {
      namestore[nameRef.relativeName] = namestore[nameRef.relativeName] || [];
      if (namestore[nameRef.relativeName].indexOf(name) < 0) {
        namestore[nameRef.relativeName].push(name);
      }
    }
    return nameRef;
  }

  function resoluteName(name, options) {
    validateName(name);
    options = options || {};
    var resolvedName = name;
    if (dependencies[name] == null && namestore[name] && namestore[name].length > 0) {
      resolvedName = namestore[name][0];
    }
    if (dependencies[resolvedName] == null) {
  ...
  this.registerObject = function(name, dependency, options) {
    var nameRef = analyticName(name, options);
   ...
   */
  this.lookup = function(name, exceptions, options) {
    name = resoluteName(name, options);
